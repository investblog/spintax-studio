...
- [x] **1. FAQ по диагностикам** — `docs/help/ru/diagnostics.md`, 17 кодов движка + 5 заметок
      Studio, 24 проверяемых примера. Плюс раздел про молчаливые поведения, которых нет ни в
      одной диагностике: `#include` работает только с начала строки, кириллическое имя
      переменной не считается переменной вовсе, неверная арность множественного числа выводит
      пустоту, неизвестный ключ перестановки становится разделителем.
- [ ] **ПОКАЗАТЬ СПРАВКУ В ОКНЕ — план, 2026-07-30.** Самый крупный незакрытый пункт R0: текст
      есть, а прочитать его в приложении нельзя — ни просмотрщика, ни пункта меню, ни строки в
      таблице. Порядок шагов, и он именно порядок:
      ...
      6. **Второй вход, кроме панели:** F1 и пункт меню на содержание — справка нужна и когда
         ошибки нет.
      7. **Язык.** В R0 справка только русская, интерфейс — на четырнадцати языках.
         Просмотрщик обязан сказать это прямо, а не показывать русский молча.

- [ ] **ОКНО «О ПРОГРАММЕ» — план, 2026-07-30.** Меньше по объёму и обязательно до первой
...
- [ ] **Английская версия** — после того, как русская устоится. По канону выходные документы
      следуют аудитории, поэтому здесь порядок обратный интерфейсному.
