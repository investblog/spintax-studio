...
2. ~~Two small product gaps~~ — **both closed 2026-08-01.** The diagnostics list has keyboard
   navigation (`DiagKeyUp` in `gui/SpxMainForm.pas`, wired where the list is built) and carries
   severity as an icon: three shapes as well as three colours, the word still in the Level
   column. Named rather than cited by line, because a line number in a backlog is stale by the
   next commit — this entry's own references went wrong within the hour.
3. **Accessibility**, and it is the slice in flight. What Microsoft's Partner Center offers is a
   SELF-declaration — *"This product has been tested to meet accessibility guidelines"* — which
   nothing verifies at certification and which we are **not ticking for R0**. The reason is
   measured, and it is the toolkit rather than our carelessness: **LCL has no accessibility
   bridge on the win32 widgetset**, so `AccessibleName` assigns, reads back correctly and
   reaches nothing.

   `docs/accessibility-baseline.txt` is the evidence — a walk of the UI Automation tree, the
   same API a screen reader reads through, regenerated by `scripts/uia-walk.ps1`. It shows the
   whole window as about twenty nameless `Pane` nodes: the editor's text, the diagnostics rows
   and the tool rail are not merely unnamed, they are not there. The same probe run against
   Notepad returns real roles, names and focusability, so this is a measurement of the app and
   not of the instrument.

   What the slice does instead is make every FUNCTION reachable through something native, fix
   what the locale box reports, and hand colour back to the system under a high-contrast theme.
   An ADR will carry the decision and what going further would cost (a UIA text provider for
   SynEdit); until it lands, the baseline file is the record.
