...
> `743a6e3`), R1-8 (спека §6 и ADR 0012), плюс раунды ревью: платформенный раскол и разбор URL,
> «локальность» как четыре разных факта, открытый `http` с ключом, редиректы, и три раунда
> Codex-гейта по R1-3 — двенадцать находок, все воспроизведены.
>
> **Следующий — R1-4, окно:** `[Generate ▾] [Fix]` на верхней полосе, панель как настройки
> профиля. Две обязанности приехали из R1-3 и в плане записаны при нём: окно зовёт
> `Invalidate` при каждой смене документа/локали/переменных/профиля, и перед применением
> сверяет `Loop.Revision = Result.Revision` — доставка и `Invalidate` живут на одном потоке,
> так закрывается последняя гонка.
>
...
- [ ] **Строки для новых ошибок — вместе с R1-4.** `leRedirected` и «этот профиль требует ключ»
      пока не имеют текста на 14 языках: окно, которое превращает `TSpxLlmError` в предложения,
      строится в R1-4. Не забыть, что редирект — сообщение, которое читатель должен понять
      («эндпоинт просит обратиться по другому адресу; поменяйте профиль, если это нарочно»).
