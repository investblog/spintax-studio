...
- [ ] **N0 — правки опубликованного, ПЕРЕД кодом или вместе с ним.** Три утверждения становятся
      ложными в момент отгрузки, а не в момент включения тумблера:
...
- [ ] **N1 — `gui/SpxHttp.pas`, обёртка над WinHTTP.** Таймауты и отмена обязательны: у рендера их
      нет, потому что он ограничен сверху, а ответ модели не ограничен ничем. `WinHttpSetTimeouts`
      в `winunits-base` не объявлен — одна строка `external 'winhttp.dll'`. Интерфейс без Windows
      в сигнатурах, реализация под `{$IFDEF WINDOWS}` (ADR 0007).

- [ ] **N2 — ключ.** `CryptProtectData` / `CryptUnprotectData` из `crypt32.dll` (в `winunits-base`
...
- [ ] **N3 — два формата запроса.** Anthropic Messages API и любой OpenAI-совместимый URL. Второе
