...
- [x] **8. Замена — СДЕЛАНА** *(спроектирована и собрана 2026-08-14; проект —
      `.agents/plans/active/find-replace.md`, владелец: «отличный план, не
      перенаворачиваем»)*. Ctrl+H / Edit → «Заменить…» раскрывают полосу поиска второй
      строкой: поле замены той же ширины, [Заменить][Заменить все]. Ядро — `SpxReplaceAllText`
      в src: ОДИН сплайс по списку от `SpxFindAll` (тот же матчер, что счётчик и навигация,
      с фолдингом движка — в этом весь аргумент против `SynEdit.SearchReplace`), без
      пере-скана ('a'→'aa' не плодится), **перекрытия пропускаются** — находчик их считает
      (шагание хочет оба «аа» в «ааа»), а метла меняет только не делящие буквы, поэтому
      «заменено» может честно назвать число меньше счётчика. Один шаг undo (доказано
      фотографиями: после Ctrl+Z заголовок без звёздочки); wholesale-редактирование несёт
      долги AiReplace (`AfterReplaceEdit`: closer line, инвалидация петли, рендер, список
      матчей). Справка ×14 — раздел «Поиск и замена» (ПЕРВОЕ описание поиска вообще: полоса
      стояла с R0 недокументированной), три подписи под гейтом цитат, доказан падением на
      сербской. 13 проверок ядра (черновик записи говорил «12» — счёт проверок тоже утверждение о свидетельстве, поймано Codex'ом). Замечено и не чинится сейчас: строка «Заменено: N» в
      статус-баре живёт до прихода вердикта рендера («valid · N ms» затирает её через
      полсекунды) — тот же класс, что у «Draft replaced» из R1-4; если начнёт мешать —
      решать классом, не точкой.

- [x] **9. Меню «Вставка» + глиф Replace — СДЕЛАНО** *(владелец, 2026-08-14: «забыл подобрать
      глиф под Replace… просится вставка конструкций вроде #set %% =, обернуть в примечание…
      можно подумать о введении новых пунктов»; проект — `.agents/plans/done/menu-insert.md`,
      утверждён с добавкой владельца: «Я бы во вставку добавил условия»)*. Пятое верхнее меню
      между Правкой и Видом: обёртки {}/[] переехали с шорткатами и глифами, НОВОЕ «Обернуть в
      /#…#/» (Ctrl+/), четыре конструкции — `#set`/`#def`/`#include`/условие — и «Вставить к
      себе» из справки (это вставка и есть). **Подпись конструкции = вставляемые байты,
      буква в букву** (один Tr-источник, дрейф невозможен по построению); ИМЯ-плейсхолдер
      латиницей во всех 14 языках — запинено побайтово (`inslang/*/…-name-is-latin`, доказано
      кириллическим именем в сербской). Глиф Replace — MDI `find-replace` (лупа + стрелка
      цикла, родня Find'у строкой выше); шрифт скачан заново и **все 31 прежние клетки
      отрендерились байт-в-байт** прежде, чем полосы были перегенерированы. Ядро в src:
      `SpxWrapCommentAllowed` (записанное заранее исключение из «спрашивает движок» — движок
      МОЛЧИТ про сбегающий `#/`), `SpxCondThenAllowed` (голая `|` / незакрытая скобка),
      `SpxTemplatePlaceholder`/`SpxCondParts` (сломанный перевод отказывает, а не вставляет
      мусор), `SpxDirectivePlan` (директива — первая на строке; пробелы перед — можно,
      текст после каретки уезжает вниз, чтобы не влипнуть в значение).

      Три измерения, стоящие записи. (1) **Теория шва опровергнута движком до кода:** «`/` в
      начале выделения склеится с `#` открывашки в `#/`» — нет: сканер ищет закрывашку ПОСЛЕ
      открывашки (`a /#/x#/ b` → `A b`, запинено), поэтому голая обёртка без пробелов и guard
      только на `#/` внутри. (2) **Ловушка brace-комментария укушена в ТРЕТИЙ раз** — `{ }`
      комментарий, цитирующий `{?name?then|else}`, закрылся на его `}` — **и в ЧЕТВЁРТЫЙ в
      тот же час**, комментарием, написанным при починке третьего и цитирующим измеренный
      рендер с `}` внутри; правило устава теперь механическое: цитата языка ИЛИ ЕГО ВЫВОДА —
      только в `(* *)`. (3) **`%name%` в таблице строк читался
      гейтом Format-плейсхолдеров как спецификатор** — турецкий `%ad%` дал «%a там, где в
      английском %n»; чекер научен съедать `%слово%` (≥2 букв между процентами) как макрос,
      настоящий `%d`→`%s` по-прежнему валит (доказано на сербской). Проверка в окне: обход
      меню кросс-процессно (5 меню, 9 пунктов Вставки, Ctrl+/ отображается, все тики целы) +
      WM_COMMAND по id с чтением сохранённых байтов — `#set` лёг своей строкой, `name`
      выделился, условие обернуло именно его, пара `/#  #/` легла с курсором внутри.

      **Codex, раунд 1 — оба finding'а ПОДТВЕРЖДЕНЫ движком и закрыты, плюс третий той же
      семьи, найденный при починке.** Первый guard смотрел только на выделение; обёртка
      ВНУТРИ открытого комментария отдавала ему свою закрывашку (`A /# x y #/ B` давал
      `A B`, после обёртки `x` — `A y #/ B`: хвост сбежал), условие внутри комментария
      теряло полрамки (`A z|else} B`), а выделение, укравшее у комментария открывашку,
      вдовило его закрывашку (`A z #/ B`). Все три воспроизведены на движке ДО веры и
      запинены рендерами. Починка: `SpxCommentOpen` (состояние комментария сканом самого
      движка) + guard'ы приняли (Prefix, Sel[, Suffix]); полный комментарий внутри выделения
      экранирует свои `|` от проверки условия (его вырезают до парсинга); незакрытая `/#` без
      закрывашки дальше — разрешена (с v0.5.0 это текст, и закомментировать его — сама
      просьба). Обе строки отказа переписаны ×14, обе фразы справки расширены ×14.

      **Codex, раунд 2 — оба finding'а снова ПОДТВЕРЖДЕНЫ измерением и закрыты.**
      (1) Граница правки, режущая существующую метку пополам, обходила все три пофрагментных
      скана: каретка между `/` и `#` живой открывашки + вставка пары дала
      `A //#  #/# x #/ B` → рендер `A /# x #/ B` — закомментированный текст ВОСКРЕС вместе
      с метками (до правки было `A  B`). Починка: `SpxSplitsCommentMark` на обеих границах +
      `TextBefore`/`TextAfter` перестали дописывать фантомный `\n` (он прятал граничный
      символ); при запрете рассечения пофрагментный скан ТОЧЕН — состояние на границе
      определяется одним префиксом. (2) Guard условия отказывал мёртвой открывашке, которую
      движок с v0.5.0 читает текстом: `y /# z` без закрывашки дальше — безопасно, разрешено,
      запинено рендером (ветка выживает дословно). Но НЕ слепым ослаблением: `|` после
      мёртвой открывашки ЖИВАЯ и отказывает по-прежнему — наивное «разрешить незакрытые»
      заэкранировало бы её (этого Codex не заметил; найдено при починке). Обёртка в
      комментарий СОЗНАТЕЛЬНО оставлена на наивном скане префикса: она ДОБАВЛЯЕТ закрывашку,
      которая оживила бы мёртвую открывашку — там отказ корректен, а не консервативен.

      **Codex, раунд 3 — за пределами двухраундового лимита, потому без пере-ревью:
      починено и заявлено в коммите** (прецедент HTML-чистилки): вставка условия БЕЗ
      выделения шла мимо split-guard'а, а у `SpxCondThenAllowed` с пустым Sel обе проверки
      рассечения были вакуумными (обе получали пустую сторону). Закрыто веткой пустого
      выделения в самой функции (запинено именем, которое Codex сам назвал:
...
- [x] **13. Вычитка английского листинга + 13 черновиков под финал — СДЕЛАНО 2026-08-15**
      *(владелец: «с вычиткой английского ты справишься лучше меня»)*. Восемь редакторских
      решений, каждое измерено: абзац ИИ без швов от наслоившихся правок; из абзаца
      раскрытия убран дубль; в описании наконец сказано про четырнадцать языков (было
      только буллетом); третий путь экспорта (по файлу на вариант) добавлен в абзац и в
      буллет 14 — он всегда был в продукте и нигде в листинге; буллет 9 заговорил словом
      справки — shuffles с честными заявками про подмножества и разделители (лист больше не
      говорит «permutation» в одном пункте и «shuffle» в другом); слабейший буллет 18
      (пересказ 17-го + аудитория) уступил место ИИ-черновику — со скоупом, который Codex
      поймал тем же классом, что вчера: «finished drafts arrive checked… applying them is
      yours». Грамматика списка в «Built for local work», «localizers» вместо
      «localization workflows». **Тринадцать черновиков в `marketing/store/` пересобраны под
      финальный английский** — новые абзацы ИИ и раскрытия (адрес support@301.st в каждом,
      чекер требует его по имени), фраза о языках, экспорт, локальная работа и шесть
      буллетов на язык; вычитанные переводы нетронутых абзацев сохранены байт в байт.
      Codex: одна находка, закрыта ×14. 20 буллетов, максимум 175 знаков, все гейты
      зелёные.

- [x] **12. Авто-применение черновика убрано — задуманный флоу возвращён** *(владелец,
      2026-08-15, из живой работы: выделил последний абзац, отнёс кусок шаблона в бриф с
      коротким промптом «насыть синонимами» — а чистый ответ затёр ВЕСЬ документ, минуя
      окно «Ответ модели»)*. Auto-apply на loClean был прикручен вместе с живой петлёй
      поверх задуманного флоу «ответ в окно, применяет читатель». Теперь: **в редактор не
      пишет ничто, кроме нажатия читателя** — чистый черновик ложится в «Ответ модели» с
      новой строкой статуса «Черновик проверен и ждёт…» (sAiReady ×14); проверка ревизии
      осталась ради ЧЕСТНОСТИ статуса (двинулся документ, пока ответ летел, — строка
      предупреждает, что вердикт был о прежнем тексте). **«Вставить в документ» получил
      семантику вставки: активное выделение ЗАМЕНЯЕТСЯ** (сценарий владельца — обогатить
      фрагмент и положить на место), без выделения — у курсора, как было. Fix туда же:
      исправленная версия ждёт в окне, её место — «Заменить документ». Справка переписана в
      трёх абзацах ×14 (глава AI: черновик ложится в окно уже проверенным; применение —
      ваше; Ctrl+Z после любой из кнопок); compliance-запись 11.16 поправлена («before it
      is offered», не «before the window applies it»).

      **Codex, два раунда, одиннадцать находок, все закрыты** (третьего раунда нет — лимит;
      заявлено здесь и в коммите). Главные: гарантия «каждый черновик проверен до
      предложения» верна только для ЗАВЕРШЁННЫХ раундов — остановленный посреди проверки
      несёт непроверенный текст, ручной путь не пре-верифицируется вовсе (документы и
      справка ×14 говорят это явно: у Stop появилась оговорка); поле ответа редактируемо, и
      «проверен и ждёт» переживало правку — теперь панель сообщает о правке, и форма
      снимает ровно это предложение (флаг, который гасит любой другой SetStatusText);
      sAiStale винил документ, а ревизию двигают и значения, и локаль, и профиль —
      переписан ×14 («изменилось то, против чего проверяли»), и stale-предупреждение теперь
      закрывает ВСЕ вердикты о сдвинутом снапшоте, не только чистый (устаревшие счётчики
      ошибок менее честны, чем предупреждение); оговорка про выделение в ru/uk/be была
      логически ПЕРЕВЁРНУТА («если только нет выделения» — сказала противоположное) —
      исправлена; контракты юнита петли и преамбула листинга перестали описывать снесённый
      авто-apply. 29032 проверки.

- [x] **11. Прогон собственными агентами-ревьюверами перед подачей — СДЕЛАН 2026-08-15**
      *(указание владельца: «прогон своим агентом-ревьювером, не внешним Codex»)*. Три
      параллельных агента: поверхность подачи, справка-против-реализации, недельный код по
      швам. **Пятнадцать находок, каждая проверена по цитате до правки; все закрыты.**

      **Поверхность подачи:** нота для сертификации всё ещё говорила «makes no network
      requests at all» — текст, который вставляется в Partner Center дословно, против пакета
      с `internetClient` (переписана под AI-сборку); политика во всех трёх копиях называла
      груз Fix «черновиком» — а Fix шлёт ТЕКУЩИЙ ДОКУМЕНТ целиком, чем бы он ни был, и в
      режиме «текст-в-шаблон» вставленный текст едет как бриф (раскрытие переписано ×3);
      ключ переживает удаление приложения — Credential Manager не чистится MSIX'ом (записано
      ×3); чекер черновиков теперь требует адрес жалоб в каждом из 13 переводов (все 13 пока
      без disclosure — обновляются после вычитки английского владельцем).

      **Справка:** вступление несло нескоупленную офлайн-гарантию «Всё здесь работает без
      сети… нечего включать» ×14 — против спеки §4.5 и собственной AI-главы 140 строками
      ниже (отскоуплено по именам функций ×14); турецкий назвал движок «motor» дважды при 51
      «makine» (класс хорватского *pogon*); «показывает первые знаки ключа, чтобы было
      видно, который» — а различает ключи ХВОСТ, и поле показывает и его (×14); **Сид** ≠
      подписи `сид`; немецкий Ctrl+Z при Strg во всём документе.

      **Код (три функциональных):** (1) **закон «wholesale SelText не достигает
      EditorChanged» был неверным обобщением** — измеренный факт про `Text :=`; SelText
      стреляет при закрытии undo-группы, и фото звёздочки это доказывало с самого начала.
      Следствие: AiReplace/AiInsert не гасили дебаунс → двойной рендер, превью без сида
      видимо перескакивало на другой вариант (погашено; три ложных комментария и устав
      переписаны — ремень оставлен сознательно). (2) **Replace All дописывал фантомную
      пустую строку каждым проходом**: `FEditor.Text` несёт терминатор, которого в последней
      строке не было, свип копирует его дословно, вставка делает из него новую строку —
      документ рос на строку за проход, Save писал байты, которых никто не набирал (свип
      переведён на `DocText`, который срезает ровно фантом и уважает настоящий хвостовой
      перевод). (3) **Кнопка Fix умирала после любой правки профиля/списка переменных**:
      снапшот двигался без рендера позади, и ничто её не перевооружало (обе ручки теперь
      перезапускают дебаунс — прецедент RuntimeChanged). Мелочь: «Привязать ключ» при
      нечитаемом адресе говорил «ключ не привязан» — новая строка ×14 называет адрес;
      обёртки {} и [] получили тот же split-mark guard, что их сёстры; тихий Continue в
      гейте цитат стал именованной проверкой. 29004 проверки, оба бинарника.
...
- [x] **10. Глифы Вставки + канал жалоб в About — СДЕЛАНО** *(владелец, 2026-08-14: «Добиваем
      ...
      **Codex, два раунда, пять находок, все подтверждены чтением и закрыты.** Главная —
      ...
      не сразу строить.** Вопросы, которые проект обязан решить до кода:
      (а) геометрия — полоса поиска живёт в шапке верхней полосы, второе поле и две кнопки
      («Заменить», «Заменить все») туда не влезают на минимальной ширине — вторая строка
      шапки по образцу помощи? (б) семантика — буквальная замена с тем же переключателем
      регистра, что у поиска, без regex в первой версии; замена внутри директив и групп —
      просто текст, движок перевалидирует на обычном render-пути, отдельной защиты не надо
      (правило группового редактора «решает движок», не список запретных символов);
      (в) «Заменить все» — ОДИН шаг undo (`BeginUndoBlock`, как у записи группового
      редактора и замены значения определения; Insert/Replace панели, вопреки первому
      черновику этой записи, идут через `BeginUpdate` — проверено Codex'ом) и
      число замен в счётчике полосы; (г) `SynEdit.SearchReplace` с `ssoReplace`/`ssoReplaceAll`
      против ручного прохода по `SpxFindAll` — выбрать измерением на длинном документе,
      помня квадратичную ловушку caret-пути; (д) строки ×14 и статья справки тем же PR.
...
Решения владельца: транспорт WinHTTP; ключ пользователя в **Windows Credential Manager**
(DPAPI-backed — так требует спека §6; ADR 0012 писался «своим файлом» и получил поправку);
Anthropic **плюс** поле для OpenAI-совместимого endpoint; `internetClient` объявляется, хотя
технически не требуется.

> ### Где мы сейчас (обновлено 2026-08-10)
>
> **План среза — [`.agents/plans/active/r1-llm-loop.md`](../.agents/plans/active/r1-llm-loop.md)**
> (локальный, не в git). Он несёт собственные поправки после двух ревью — читать его, а не
> восстанавливать порядок работ отсюда.
>
> **Закрыто:** N0, N1/R1-1 (провайдеры), N2/R1-2 (ключ в Credential Manager), R1-3 (петля,
> `743a6e3`), **R1-4 (окно, 2026-08-13)** — кнопки, панель-настройки, гранты по оригину,
> обе унаследованные обязанности, — R1-8 (спека §6 и ADR 0012), плюс раунды ревью:
> платформенный раскол и разбор URL, «локальность» как четыре разных факта, открытый `http`
> с ключом, редиректы, и три раунда Codex-гейта по R1-3 — двенадцать находок, все
> воспроизведены.
>
> **Закрыт и R1-5 (2026-08-13):** канал жалоб, disclosure, отметка Partner Center и
> процедура — четыре обязанности 11.16, счётчик ссылок стал `exactly three` одним коммитом
> со страницей.
>
> **Закрыт и R1-7 (2026-08-14): справка на 14 языках — глава «AI-черновик»** в `studio.md`
> (слаг `ai`, между панелями и групповым редактором), гидом, не описанием кода (указание
> владельца). Путь от вставленного текста: «Текст для переделки» → Generate; «ответу не
> верят — его проверяют» (петля правит сама, всё ждёт в поле ответа). *(Авто-применение
> чистого черновика, описанное здесь изначально, убрано 2026-08-15 — см. пункт 12 UX-плана;
> справка переписана тогда же.)* Fix;
> подключение — Формат/Эндпоинт/Модель/Авторизация, **где взять ключ у обоих провайдеров**
> (console.anthropic.com → API keys; platform.openai.com → API keys + включённый биллинг;
> OpenAI-compatible как семейство: OpenRouter, локальные Ollama / LM Studio без ключа);
> Credential Manager и показ начала ключа; согласие с именем получателя, отказ редиректа,
> `http` только на эту машину, галка «Отправка разрешена» как выключатель даже на лету;
> офлайн-петля — «та же петля без сети». **Ложное абсолютное убрано на всех 14:** старая
> справка утверждала «приложение не разговаривает с моделью и не держит ключ» — верно в R0 и
> ложь с ADR 0012. Подписи интерполированы из таблиц строк (не переписаны на глаз);
> кейс-абзац каждого языка переиспользован дословно; словарь §4.5, `TestLocalityWords`
> зелёный; `HELP_UI_QUOTES` +4: Generate, Fix, Привязать ключ, Отправка разрешена (0..29).
> Одна ловушка генератора: в этом подмножестве markdown нет курсива — `*API keys*` уронил
> make-help на первом же прогоне, звёздочки сняты ×14.
>
> **Codex прочитал главу ПРОТИВ реализации — семь находок, все подтверждены по строкам кода
> и закрыты во всех 14 разом** (главы регенерируются из одного набора шаблонов). Две
> главные: «отправка остановится даже на лету» обещала больше, чем даёт отмена (транспорт
> читает флаг до набора номера и между чтениями ответа — `SpxHttp.pas:452,486`; теперь
> фраза говорит измеренное: новое не отправляется, летящий ответ никогда не применяется);
> и «больше ничего не уходит» прятала инструкции самого промпта, диагностику при починке и
> ключ в заголовках — раскрытие теперь называет всё. Меньшие: ключ и согласие привязаны к
> схеме+хосту+порту, не к строке эндпоинта (смена пути грант НЕ сбрасывает); подсказки
> моделей есть только для Anthropic; офлайн-петля — те же ПРОМПТЫ, не та же защита (движок
> судит после вставки, петлю крутит читатель); редирект «отвергается, а не выполняется» —
> без обещания показать назначение (2048-байтный буфер Location не перечитывается);
> турецкий писал «на разных языках» вместо регистров, голландский «bij name», английский
> удвоил артикль. **Раунд 2 держал уже исправленные фразы к тому же стандарту и нашёл ещё
> три:** раскрытие всё ещё прятало имя модели и потолок длины ответа, которые едут в каждом
> запросе; фраза о разрешении говорила «на адрес» абзацем ниже фразы о ключе, уже выучившей
> «схема+хост+порт»; и восточнославянские копии звали низ панели «подвалом». Всё закрыто,
> по четырнадцать разом. 28945 проверок.
>
> **Следующий:** R1-10 (WACK заново — первый кандидат с
> исходящими соединениями) и R1-9 (сайт и подача, последним, решением владельца).
>
> **Проверка после пуша — `gh run list`.** Локальный гейт собирает Windows; CI собирает ubuntu
> и macos тоже, и оба раза за эту сессию ломалась именно не-Windows ветка.

- [x] **R1-3 — `TAuthoringLoop`: Generate → Verify → Fix** *(2026-08-12, `743a6e3`).*
      `gui/SpxLlmThread.pas` — свой поток, не UI и не движковый; каждый вердикт — движка, через
      новый тип запроса в `SpxEngineThread`: verify-джобы едут своей FIFO-очередью и не
      вытесняются превью (обычный `Post` — latest-wins, один набранный символ молча выбил бы
      Verify; доказано превращением очереди в latest-wins — падает ровно одна именованная
      проверка). Таблица исходов из плана выписана в коде и в наборе, с пятой строкой, которой
      в плане не было: **ошибка в подключённом фрагменте — `loClosureError`**, показать и не
      применять, потому что «чистый вердикт» спеки — всё замыкание, а перегенерация не чинит
      файл на диске. Снимок запроса глубокий (dyn-array в FPC алиасится — `Copy()`), ревизия
      сверяется перед каждым повтором, перед доставкой и ещё раз В доставке (главный поток —
      тот же, откуда приходят `Cancel`/`Post`, поэтому там гонка кончается); результат везёт
      ревизию для повторной сверки окном при применении. Находки без координаты уходят модели
      отдельным списком — а когда размещённых нет вовсе, СМЕНЯЮТ плейсхолдер порта «верни
      шаблон без изменений» (последнее вхождение якоря `ERRORS:`-блока: пользовательский текст
      — шаблон, многострочная заметка переменной — может стоять только раньше настоящего).

      Три раунда Codex-ревью до коммита — первый срез под новым правилом устава. Двенадцать
      находок, каждая воспроизведена; две ушли за пределы диффа: `SpxHttpSend` читал флаг
      отмены только МЕЖДУ чтениями, так что Stop до начала обмена всё равно отправлял промпт и
      ключ и ждал таймаута — теперь есть нулевое чтение на всех трёх платформенных ветках; и
      гейт на это нулевое чтение сам был пойман на измерении не того: он звонил на реальный
      адрес и ПРОХОДИЛ со снятой правкой, потому что позднее чтение отвечало `heCancelled`
      после того, как передача уже случилась. Теперь он целится в закрытый loopback-порт:
      зелёное состояние не набирает вовсе, сломанное падает мгновенно и офлайн.

      **Fullwidth без ошибки измерен как недостижимый** — пробником через настоящий движок:
      каждая форма кривой арности (runtime-счётчик, `#set`, `#def`; en и ru) несёт
      `plural.arity` error вместе с fullwidth. Ветка остаётся, как записана в таблице спеки, —
      оборонительной; набор утверждает старшинство (ошибка побеждает fullwidth, петля чинит).
      Пустые пробы без ошибки достижимы двумя путями и оба в наборе. Учёт денег: попытка
      Fix тратится в момент ПРИХОДА ответа — оплаченное оплачено, даже если Stop лёг в ту же
      секунду; раунд, умерший в транспорте, бюджета не ест. 26224 проверки в обоих бинарниках;
      восемь новых гейтов доказаны на сломанных состояниях.
...
- [x] **«Локальность» разведена на четыре разных факта** *(2026-08-10, ревью).* Слово
      «локально» в спеке означало одновременно адрес, отсутствие ключа, отсутствие сети и
      отсутствие нашей инфраструктуры — и часть из этого уже разъехалась с кодом и с политикой.
      Нормативное правило теперь одно: **спека §4.5, «Endpoint, авторизация и „локальность“»**,
      со словарём (`offline core` / `user-configured endpoint` / `managed endpoint` /
      `localhost preset`). Противоречия убраны в §1, §4.1, §6, §10, §11 и в строках релизов;
      историческое утверждение про R0 (`offline-only`) оставлено — оно привязано к версии и
      остаётся правдой.

      **Правка одной только спеки оставила бы продукт неверным, поэтому правились три слоя:**

      - **Код.** `SpxLlmIsLocal` удалён. Он разрешал пустой ключ для loopback-адреса и отказывал
        всем прочим — неверно в обе стороны. Появился `TSpxLlmAuth` (`laNone` / `laApiKey` /
        `laServiceToken`) в профиле; заголовок и преflight смотрят на него, а не на hostname.
        Профиль с `laNone` не шлёт ключ, даже если ключ лежит в конфиге.
      - **Политика, все три копии.** «If the endpoint you configure is a local one … then
        nothing leaves the computer at all» — не измерено и неверно: на том же порту отвечает
        прокси или туннель. **Не опубликовано** (витрина всё ещё несёт офлайновую политику), то
        есть поймано до того, как стало опубликованной ложью.
      - **Гейт.** Маркер требовал `localhost:11434` в каждой копии — то есть **прибивал ложное
        утверждение**. Гейт держит три копии в согласии, но не ручается за их содержание;
        теперь он пришпилен к исправленному утверждению.

      **И редиректы оказались утечкой ключа, а не гигиеной.** WinHTTP по умолчанию идёт за 3xx
      и переотправляет заголовки. Измерено живьём в обе стороны: зонд слал
      `x-probe-secret` на редиректор и **получал его обратно из эхо-ответа другого хоста**; с
      `WINHTTP_DISABLE_REDIRECTS` — `status = 302` и заголовок никуда не уехал. 3xx возвращается
      как `heRedirected` с адресом из `Location`.

- [x] **Ключ мог уйти открытым `http` на чужой хост** *(2026-08-10, ревью плана, P0).* Прямое
      следствие правки «локальности»: авторизация перестала зависеть от адреса, значит
      `api-key` + `http://example.com` стало достижимой конфигурацией, и ключ поехал бы в
      открытом виде. Спека §4.5: **открытый `http` — только на эту машину, для всего остального
      обязателен `https`**, и это свойство ТРАНСПОРТА, а не обещание локальности обработки —
      пакет на 127.0.0.1 не пересекает сетевой интерфейс, и это верно про IP-стек, что бы за
      ним ни слушало. `heInsecure` в транспорте, `leInsecure` в петле **до сборки тела**.
      Следствие записано: сервер модели в локальной СЕТИ по открытому http тоже отказывается —
      если понадобится, это отдельная явная настройка со своим согласием, а не ослабление.

- [x] **Sweep по терминологии + гейт** *(2026-08-10).* Ревью назвало три места, `grep` нашёл
      шесть: заголовки `SpxLlm`/`SpxHttp`, два комментария набора, комментарий самого гейта
      приватности и — самое опасное — готовый к вставке абзац в
      `docs/publish/network-slice-edits.md`, документе, чьё назначение «вставить при подаче».
      `TestLocalityWords` запрещает четыре утверждающие фразы во всём, что доедет до читателя;
      `tests/`, `docs/decisions/` и этот файл намеренно вне области — цитировать неверную фразу
      там и есть работа. Доказан возвратом фразы в политику: одно именованное падение.

- [x] **`ErrName` кончался `else Result := 'leCancelled'`** *(2026-08-10).* Каждый новый член
      enum читался как «отменено». В этот раз дало падение и час на поиск; с тем же успехом
      дало бы молчаливый неверный проход. Все члены названы, `else` — жалоба вместо догадки.
      Тот же класс, что `if code = 'en' then X else Y` в уставе.

- [x] **R1-4 — окно: `[Generate ▾] [Fix]`, панель как настройки профиля** *(2026-08-13)*.
      Кнопки в верхней полосе справа от seed-группы; `Generate` в полёте становится `Stop`
      (слот меряется по ШИРШЕЙ из двух надписей, чтобы смена не двигала соседей), `Fix`
      включён только при error-строках самого документа, ▾ — меню «Настройки ИИ… / Копировать
      промпт» (ручной путь §11 остаётся с парадного входа). Панель несёт профиль подключения:
      формат, endpoint, модель, режим авторизации (`none`/`api-key`; `service-token`
      распознаётся в файле, но не показывается — §6 резервирует), ключ write-only в
      Credential Manager под `byok/default`. Обе обязанности R1-3 исполнены: `Loop.Invalidate`
      на каждой смене документа/локали/переменных/allow-list/профиля (wholesale-присваивания
      текста, которые не доходят до `EditorChanged`, инвалидируют сами), и сверка
      `Loop.Revision = Result.Revision` на главном потоке перед применением `loClean`;
      teardown строго по контракту `TSpxAuthoringLoop.Create`.

      **Личность профиля — через ОРИГИН, не через строку адреса.** `SpxLlmOrigin` — это
      scheme+host+port, в нижнем регистре, с выписанным дефолтным портом; ключ и согласие
      `ai.network` (10.5.2) — оба гранты НА ОРИГИН: смена пути — тот же получатель, смена
      scheme/host/port — другой, и ключ отвязывается (хранится, но не отправляется —
      опечатка в адресе не уничтожает его), а согласие гаснет и диалог называет нового
      получателя. Настройки хранят режимы СЛОВАМИ (`ai.auth=api-key`); три гейта доказаны
      на сломанном состоянии: убранный `LowerCase` хоста, порядковый номер вместо слова, и
      прямое присваивание out-параметра `FromWord` (он пишет его и при False — файл R0 без
      `ai.*`-ключей обязан читаться как «сеть выключена», а не как Anthropic-профиль).

      36 строк на 14 языках, термины измерены по справке (движок/рушій/Maschine/motor…);
      «Senden erlaubt» вместо полного предложения — немецкий обрезался на фотографии, и
      подпись сокращена во всех четырнадцати, полное описание живёт в диалоге согласия.
      Две ловушки инструмента за один заход: `TCheckBox` РОЖДАЕТСЯ AutoSize, и ширина ему из
      Resize-пути — это `ChangeBounds loop detected` модальным диалогом до первого окна (то
      же правило устава, третий экземпляр); и DllImport W-функций без `CharSet=Unicode`
      маршалит заголовки в один символ — зонд честно отвечал «окна нет» про окно, которое
      стояло на экране.

      **Два раунда Codex-гейта, девять находок, все воспроизведены и исправлены.** Худшие —
      про ВОЗРАСТ снимка: пара (текст, строки) для Fix отстаёт от редактора на один рендер
      НАМЕРЕННО, а ревизию запрос брал текущую — правка между рендером и нажатием Fix
      применила бы починку старого текста поверх нового (теперь у пары своя ревизия,
      записанная в момент поста рендер-джобы, и Fix гаснет, пока пара отстаёт); отзыв
      согласия не останавливал обмен в полёте (`Cancel` — первым действием, до записи
      настроек); `service-token`-профиль из чужого файла прочитал бы BYOK-ключ (пространство
      секрета выбирается по режиму, и панель гасит Attach/Forget для зарезервированного
      режима); `AiReplace`/Save/Reload set/GSA-импорт двигали контекст без инвалидации; и
      IPv6-origin терял скобки — а его показывает диалог согласия как получателя. Одна
      позиция удержана: тестов оконной обвязки в консольном наборе нет и не будет — plain
      fpc не собирает LCL-формы, окно в этом проекте проверяется зондами и фотографиями, а
      отдельный GUI-бинарник беклог уже взвесил и отложил.

- [x] **R1-5 — четыре обязанности политики 11.16** *(2026-08-13)*. Канал жалоб: Help →
      «Report inappropriate AI output…» (14 языков) → mailto:`support@301.st` с продуктом и
      версией в теме — читателю открывается ЕГО почтовый клиент, приложение не отправляет
      ничего (та же линия, что у двух марок: ссылка — не запрос). Третья ссылка приехала
      ОДНИМ коммитом со страницей: счётчик стал `exactly three`, абзац политики называет все
      три места во всех трёх копиях, и заодно вычищено противоречие, которое два ревью не
      видели — «Changes to this policy» четыре дня обещала фичу «в будущей версии» под
      шапкой, которая её уже описывала (маркер пинит фразу НАРУЖУ). Disclosure «live
      generative AI» — в описании листинга, с гейтом на фразу и на адрес; правило
      «R0 does not call an AI service» из описания ушло и запинено наружу. Отметка в Partner
      Center и порядок реакции на жалобы — `docs/publish/ai-content-report.md` (обязанность 2
      — действие на форме, обязанность 4 — процедура, записанная до первой жалобы). Четыре
      новых гейта доказаны падениями; счётчик ссылок по дороге поймал СОБСТВЕННЫЙ комментарий
      правки («counts the OpenURL( sites» — сканер читает текст, не синтаксис). Ещё одна цена
      сессии: `git checkout --` как «восстановление» стёр незакоммиченные правки двух файлов
      — восстановлены из контекста; откатывать temp-ломку гейтов надо ЗАМЕНОЙ строки, никогда
      не checkout, пока срез не закоммичен.

      Два раунда Codex: одиннадцать находок, все воспроизведены и исправлены. Класс, который
      стоит запомнить: **опубликованная фраза, верная для облачного профиля, была ложной для
      дефолтного** — «your own key, on your own provider account» стояло безусловным в
      листинге, трёх копиях политики и фиче 17, а дефолтный профиль — keyless localhost
      (`laNone`); всё переведено в кондиционал («when the endpoint uses them»), и туда же
      «their policy applies» (утверждало, что политика провайдера СУЩЕСТВУЕТ — self-hosted
      её не имеет; теперь «any policy the operator states applies», маркер гейта переехал за
      фразой). Гейт disclosure первой редакции удовлетворялся МОЕЙ ЖЕ пометкой в шапке файла
      — пересобран на секцию Description со сложенными переносами и доказан падением; гейт
      mailto получил терминатор адреса (префикс благословил бы `support@301.st.evil`); маркер
      report-пункта заякорен посекционно. И BOM: `Set-Content -Encoding utf8` в PS 5.1 пишет
      UTF-8 С BOM — все правленые файлы проверены и вычищены.

- [x] **Нижний блок уходит вместе с рейлом, пока открыта помощь** *(2026-08-13, владелец
      заметил то, что два раунда ревью не увидели: «зачем нам окошко AI на окне помощи?»)*.
      Это было НЕ задумано — унаследовано: маршруты помощи прятали слайд, полосу и рейл, а
      `FBottom` не трогал никто, и все четыре панели стояли под страницей руководства про
      документ, которого на экране нет. Пока там были пустые сетки, никто не спотыкался;
      R1-4 поставил туда форму с endpoint и полем ключа, и стало видно. Правило — то же,
      что у рейла, и решается в том же месте (`LayoutTopStrip`, от истины): выбор читателя
      живёт в `FBottomWanted`, видимость = выбор ∧ не-помощь. `SavePrefs` и галки View-меню
      читают ВЫБОР — иначе спрятанная ради помощи панель записалась бы как закрытая
      читателем. Пункт View-меню при открытой помощи закрывает её и показывает панель
      (действие меню обязано иметь видимый результат — правило группового редактора).
      Измерено зондом: F1 → блок ушёл, клик по закрытию помощи (TOPMOST, владелец точки
      сверен по PID) → блок вернулся тем же выбором.

- [x] **Строки для новых ошибок — вместе с R1-4** *(закрыто выше, 2026-08-13)*. Все члены
      `TSpxLlmError` получили свои предложения (`LoopErrSentence` — каждый назван, else —
      слова провайдера), включая три обязательных: редирект с адресом, `leInsecure`, и
      «профиль требует ключ» — плюс отдельное «эндпоинт изменился, введите ключ заново» для
      отвязанного ключа.

- [x] **UX-проход по AI-панели** *(2026-08-13, по прямому указанию владельца: «сам вначале
      сделай UX ревью и поправь по нему», до документации R1-7)*. Четыре правки, все из
      пути «у меня есть текст»: (1) поле «Модель» стало редактируемым комбо с датированными
      подсказками точных id (`SpxLlmModelSuggestions`, только для Anthropic-формата; список —
      подсказка, не валидатор, и это записано на функции); (2) у колонки брифа два режима, и
      её заголовок — переключатель: «Текст для переделки» (по умолчанию; бриф собирается
      вокруг вставленного текста хостом — `SpxComposeFromTextBrief`, вне порта и его
      байтовых гейтов, по-английски, потому что каркас промпта английский при любой локали)
      и «Бриф» (свободная форма, как было); (3) Generate/Stop и Fix переехали ИЗ верхней
      полосы В панель, в ask-строку — «вся работа в одном окне, а кнопка в другом» больше
      не так; кнопки панели стреляют событиями, гейт согласия/ключа/петли остался у формы;
      меню ▾ умерло (его пункты — сама панель); (4) строки подключения опустились подвалом
      панели — настройки трогают один раз, работа идёт каждый день. Пустой бриф в режиме
      текста говорит «вставьте текст», не «напишите бриф». Два новых строковых id ×14.
      Гейты: список подсказок запинен по трём wire-строкам, обёртка — по началу инструкции,
      дословности источника и пустоте на пустом входе; оба доказаны именованными падениями.
      Не сделано и записано: режим брифа не переживает перезапуск (ai.brief-mode в
      настройках — при первом же спотыкании).

- [x] **Живой прогон петли — пройден** *(2026-08-14, по приглашению владельца, его боевым
      ключом; запись целиком — в плане, Verification п.4)*. Anthropic Messages,
      `claude-opus-5`, режим «Текст для переделки»: от вставки текста до заменённого
      шаблоном документа — меньше 20 секунд, `loClean` с первого захода, превью чистое,
      грид подхватил переменную, которую модель сама определила через `#def`. И это НЕ
      «законно по промпту», как чуть не записалось сюда: hard rule канонического промпта
      говорит «Use only the names given in ALLOWED VARIABLES. Never invent one»
      (SpxPrompt.pas:255) — модель контракт нарушила, а толерантный host и движок приняли,
      потому что для рендера локальный `#def` валиден. Это «никогда не верить
      output-контракту» в живой природе — и находка ВВЕРХ: канонический промпт внутренне
      противоречив (учит `#def`, но запрещает изобретать имена) — вопрос к пакету
      `spintax-js/authoring-prompt`, не к порту (порт обязан совпадать побайтово).
      Урок достался зонду, не приложению: первая попытка вставила текст в ДОКУМЕНТ —
      панель была закрыта, и координата поля оказалась редактором; правило — открыть
      панель с рейла и доказать курсором (ask-строка отвечает ARROW, редактор — IBEAM)
      до первого клика. Живьём не покрыт OpenAI-совместимый формат (`Authorization:
      Bearer` — другой заголовок и тело); закрывается одним прогоном с ключом
      OpenAI/OpenRouter, локальная модель без ключа проверит тело, но не Bearer.

- [x] **Вставленный HTML чистится до подачи в модель** *(2026-08-13, владелец: «чистить
      стили до подачи и оставлять только те теги, что наш движок сможет переварить»)*.
      `SpxCleanSourceHtml` (в SpxPrompt, своё — вне порта): остаются теги, которыми продукт
      сам говорит в своих страницах (`p br a b strong i em u tt code ul ol li h1..h6`), у
      `<a>` выживает только `href` — единственный атрибут, который СОДЕРЖАНИЕ; `<script>/
      <style>/<head>` глотаются с содержимым; остальное — включая `<span>`, `<font>`,
      офисный `<o:p>` и неизвестные тэги — разворачивается до текста (блочные оставляют
      перенос, ячейки таблиц — пробел); сущности декодируются; вёрсточные отступы строк
      срезаются. **Ворота — распознанный тег, не символ `<`:** проза с `a < b`, буквальным
      `R&amp;D` или случайным `<j>` возвращается байт-в-байт, и это утверждение — первый
      гейт, доказан именованным падением (ворота снимались — проза «чистилась»). Чистка
      стоит в `Brief()` режима «Текст для переделки», поэтому ручной «Copy prompt» несёт
      тот же очищенный источник — читатель ВИДИТ, что ушло. Восемь проверок, по одной на
      решение.

      Codex-раунд: шесть находок, каждая сперва воспроизведена именованным падением. Две
      ломали сами ворота — сканер-калитка шагал по байту и ЧИТАЛ СКВОЗЬ комментарий и
      кавычки атрибутов, так что `<p>`, процитированный в прозе внутри `<!-- -->` или в
      `title="<p>"`, включал чистку (ворота теперь ХОДЯТ как чистильщик: мимо целого
      комментария, мимо всего распарсенного тега); `href` находился подстрокой — `data-href`
      становился живой ссылкой, а кавычка в значении протаскивала атрибут мимо белого
      списка при пере-эмиссии (`href='x" onclick="evil'`) — имя теперь точное, значение с
      кавычкой роняет ссылку целиком; `</styles>` закрывал `<style>` по префиксу; Copy
      prompt проверял сырой текст и копировал промпт с пустым брифом там, где сетевой путь
      отказывает; суррогатная сущность кодировалась в невалидный UTF-8; и по копии
      LowerAscii на каждый `<style>` — квадрат. **Урок тот же, что у санитайзеров всюду:
      прогонять и ЧЕРЕЗ ворота, и МИМО ворот — дыры были не в чистке, а в решении, когда
      она включается и что она пере-эмитирует.**

      Второй раунд — ещё шесть того же семейства, и ДВЕ из них были в ПЕРВЫХ ПОЧИНКАХ:
      однограничный `href`-фикс дал `hreflang` спрятать настоящий `href` (теперь обе
      границы, и неудачный кандидат продолжает поиск), а точное закрытие по `HtmlNameChar`
      пропускало `</style.foo>` (имя закрывающего тега кончается на пробеле, `/` или `>`).
      Плюс: незакрытая кавычка тега отдавала воротам своё нутро как «тег» (похоже-на-тег-
      но-не-парсится теперь ОБРЫВАЕТ скан ворот), doctype с `>` в кавычках, `<svg/>`
      глотал текст за собой, и типографские сущности (тире, многоточие, «ёлочки», кавычки)
      не декодировались. **Починка санитайзера — новый санитайзер: ей положен тот же
      второй взгляд, что и оригиналу.** Все двенадцать воспроизведены падениями до правки.

      Третий раунд упёрся в лимит пере-ревью (два), и его четыре находки починены уже без
      нового круга — воспроизведены, как и все: второй проход не получил стоп на
      незакрытом теге, который только что получили ворота (теперь хвост отдаётся ТЕКСТОМ в
      обоих); имя тега кончается на разделителях токенизатора, так что `<p.class>` —
      неизвестный тег, а не `<p>` с хвостом; слэш в незакавыченном значении атрибута
      принадлежит значению, а не самозакрытию; и form feed — тоже пробел HTML (вместо
      четырёх рукописных наборов границ — общая пара `HtmlWs`/`HtmlNameEnd`). Итого
      двадцать шесть проверок на один чистильщик и шестнадцать находок за три раунда.

- [x] **Поле «Модель» — дать выбор, а не только свободный ввод** *(закрыто UX-проходом
      выше, 2026-08-13)* *(пометка владельца,
      2026-08-13: «Пользователь точно споткнется»)*. Строка модели уходит в тело запроса
      дословно (`SpxLlmBuildBody`, `root.Add('model', ...)`), и на опечатку отвечает провайдер
      своей ошибкой — то есть первый контакт пользователя с фичей упирается в точный API-идентификатор,
      который негде подсмотреть (`claude-opus-5`, не «Opus 5» и не имя с датой). Форма: редактируемый
      комбобокс (`csDropDown`) вместо `TEdit` — для формата Anthropic список известных id как
      ПОДСКАЗКА, свободный ввод остаётся; для OpenAI-совместимого формата списка быть не может
      (имена определяет сервер — Ollama, прокси, что угодно), там поле остаётся пустым списком.
      Зашитый список обязан нести дату снятия и никогда не быть валидатором: правда о том, какие
      id существуют, — у провайдера, и список устареет молча (тот же класс, что `ENGINE_CODES`, —
      рукописная копия чужих констант; здесь сверить не с чем, поэтому подсказка, не гейт).
...
- [x] **N6 — `internetClient` в манифесте** *(сделано 2026-08-14)*: объявлен с комментарием,
      почему технически не нужен (medium IL) и объявляется ради витрины; гейт пинит пару
      capability ТОЧНО: обе названы, а число ЭЛЕМЕНТОВ в блоке (любых имён — Codex показал,
      что именованные счётчики пропускают `<DeviceCapability>`) закреплено через счёт `<`
      по тексту без комментариев; доказан подсадкой третьего элемента с переносом строки
      перед `Name`. Попутно **потерялась одна сборка пакета на ловушке
      `--` в XML-комментарии** — прозаичное тире в языковом комментарии от 2026-08-09, ни
      разу не проходившем через MakeAppx («expected '>'» на колонке, как и предупреждала
      шапка файла); теперь суита ВЫРЕЗАЕТ комментарии и держит каждое тело свободным от
      пары дефисов (и от дефиса в самом конце — `x--->` тоже незаконен), а остаток — от пар
      и незакрытых скобок; именованные проверки вместо падения сборки.

- [x] **N7/R1-10 — WACK: предварительный прогон СНЯТ 2026-08-14**, формальная запись — по
      конвенции против артефакта следующего тега. Локальный кандидат (дерево на 0.1.1.0 +
      internetClient + 14 языков манифеста, впервые через MakeAppx):
      **`OVERALL_RESULT=PASS`, `PARTIAL_RUN=FALSE`** — объявление internetClient не сдвинуло
      ни один обязательный тест. Опциональный Blocked Executable Files даёт ТРИ находки —
      одну ссылку на API и две подстроки
      (было две): ShellExecuteW (намеренное действие ссылок), `reg` (HTML-entity), и новая
      `dnx` — измерена до объяснения: один хит в exe, внутри таблицы 4-байтовых смещений с
      соседями `8nx`/`Xnx`/`lnx` — совпадение байтов, не ссылка на инструмент. Запись —
      `docs/release-validation.md`, там же заметка для certification notes (демо-аккаунт не
      нужен, 10.3.1). **Открытым остаётся номер версии:** тег `v0.1.1.0` существует с
      2026-08-08 и потрачен (валидирован, не подан, дерево ушло на ~110 коммитов) — следующий
      тег обязан быть новым; `0.1.2.0` или `0.2.0.0` — решение владельца.
...
## PAD file, for the software portals (opened 2026-09-06)

Published at <https://spintax.studio/pad.xml>, generated by `scripts/make-pad.py` from
[`pad-listing.md`](pad-listing.md). Written for a SoftPortal submission; PAD is read by many
portals, so it is a channel rather than one form.

**Generated, not written.** The version comes from `VERSION`, the fourteen languages from the
package manifest, the download size from the release asset measured at generation time, and
the descriptions from the copy file. The generator refuses to write a PAD whose descriptions
overflow their limits in either language, whose `Program_Language` disagrees with the
manifest, or any of whose URLs does not answer.

**Three languages, not one question.** `Program_Language` is what the PROGRAM speaks
(fourteen); `Program_Descriptions/<English|Russian>` is what the DESCRIPTION is written in
(two); the `Char_Desc_*` limits apply inside each block, and Russian at 45 characters is the
