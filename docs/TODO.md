...
- [ ] **A0 — эталоны промпта.** Скрипт снимает `systemPrompt` / `userPrompt` с JS-сборщика по
...
- [ ] **A1 — порт сборщика на Pascal.** `SpxPrompt.pas`: `SpxBuildAuthoringPrompt`,
...
- [ ] **A2 — петля в окне.** Бриф → промпт (скопировать) → вставить ответ модели →
...
- [ ] **A3 — справка на срез, в том же PR.** Правило проекта: когда срез меняет то, что
...
- [ ] **Сеть — отдельный срез и отдельный ADR, не этот.** Встроенный провайдер и ключ
      пользователя означают: HTTP и TLS в Pascal, хранение ключа, правку буллета 17 (он сейчас
      обещает обратное), новую редакцию privacy policy в ДВУХ местах (сайт и замороженный снапшот
      на CDN Microsoft) и новую декларацию о сборе данных при сертификации. Открывать это, когда
      офлайновый срез стоит и работает, а не раньше.
