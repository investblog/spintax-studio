...
>
> **Закрыто:** N0, N1/R1-1 (провайдеры), N2/R1-2 (ключ в Credential Manager), R1-8 (спека §6 и
> ADR 0012), плюс два раунда ревью: платформенный раскол и разбор URL, «локальность» как четыре
> разных факта, открытый `http` с ключом, редиректы.
>
> **Следующий — R1-3, `TAuthoringLoop`** (Generate / Verify / Fix). В плане у него четыре
> обязательных условия, добавленных ревью и НЕ выводимых из кода: Verify идёт через
> engine-поток (обычный `Post` вытесняет джобы), петля держит неизменяемый снимок, у профиля
> должна быть устойчивая личность, и у Verify выписана таблица исходов.
>
