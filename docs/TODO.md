...
- [ ] **СЛЕДУЮЩИЙ СЛОЙ СПРАВКИ — план, 2026-07-31.** Просмотрщик сделан (ADR 0008); это про то,
      ...
      **Общее правило, которое дороже списка.** Всё, что держит эту справку честной, жёсткое:
      ...
      1. **Пары «не так → так», и это закрытие живой дыры, а не украшение.** Глава «Как выглядит
         ...
         Механизм даром: генератор уже держит **словарь информационных строк у ограждений** и
         падает на незнакомой. Добавить ` ```spx-good ` — одна запись, после чего контракт
         становится: статья к коду обязана иметь пример, порождающий этот код (уже есть), а
         пример в `spx-good` обязан не порождать **ни одной** строки (новое).
      ...
      2. **«Вставить к себе».** Пример становится частью документа пользователя.
...
- [ ] **Мелочи, оставшиеся от ревью справки 2026-07-31.** Обе названы и обе не сделаны:
      Escape справку не закрывает (выходы — инструмент рельса и крестик тем); и в чтении блока
      условий генератор обрезает пробелы у значения, а набор нет — сегодня не срабатывает, но
      значение `include` с хвостовым пробелом проверялось бы против одного текста, а ехало бы
      другим.
