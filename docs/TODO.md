...
- [x] **Английская версия FAQ по диагностикам** — `docs/help/en/diagnostics.md`, те же 22 статьи,
      ...
      **Перевода нет ни строки — всё перемерено под `en`,** и локаль меняет не оформление, а сами
      ...
      так что раздел про арность пришлось вывернуть целиком. Плюс раздел, которого в русском быть
      не может: **экранирование сокращений**. `Ltd. our prices` оставляет `our` строчным, а
      контрольное `Xyz. Our prices` — прописным; список экранируемых слов английский
      (`etc vs mr mrs ms dr prof sr jr inc ltd co corp no st ave blvd`), диагностики нет, заметить
      можно только по выводу.
