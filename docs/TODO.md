...
- [ ] **2. Справочник языка** — конструкции по одной, с проверяемыми примерами. Писать после
      редактора групп: часть этого он покажет руками.
- [ ] **3. Само приложение** — две панели, сид и реролл, варианты и дедуп, экспорт. Этот же
      текст уходит в описание для Store.
- [ ] **Показать справку в окне** — вкладка «Диагностика» должна открывать статью по коду
      строки (двойной щелчок). Требует GUI-слоя; markdown уже лежит рядом.
