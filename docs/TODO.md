...
> владельца). Путь от вставленного текста: «Текст для переделки» → Generate → черновик заменяет документ,
> один Ctrl+Z назад; «ответу не верят — его проверяют» (петля правит сама, документ заменяет
> только чистый черновик, остальное ждёт в поле ответа, правки читателя не затираются); Fix;
